...
| [SQL](sql.md) | `sql-protocol` / `sql-query` / `sql-orm` / `sql-migrate` (Engine · Core · ORM · Alembic) |
...
| [AI agent](ai-agent.md) | `ai-agent-protocol` **0.2.2** (`run-ai-agent`; `:memory` + `:steering`; `/mcp` sampling; not `run-agent`) |
