...
| **Proxy** | `http-proxy-config` CLOS | (1) `configure-proxy` manual URL/alist (2) `configure-proxy-script` manual PAC (3) `load-proxy-system` = env + Win registry + OS PAC/WPAD; `resolve-proxy` → URL / NIL / `:system`. |
