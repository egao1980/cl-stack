...
| **MCP** | Host hook: `mcp-protocol:mcp-client-sampling-handler` → `generate`. **No** `llm-protocol/mcp`. No second `create-message`. |
...
| Capability | `llm-protocol/capability` (`make-llm-catalogue` / `complete`) |
