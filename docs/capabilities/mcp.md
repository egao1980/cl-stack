...
**Spec surface (GFs + CLOS, even if backend I/O is thin):** sampling (`create-message`, `mcp-sampling-request`), elicitation, roots, completions, subscriptions, logging, progress, MRTR `input_required`. Host `mcp-client-sampling-handler` is [`ai-agent-protocol/mcp:make-mcp-sampling-handler`](ai-agent.md) (`generate`) — **not** `llm-protocol`, and **not** the [blackboard](blackboard.md) loop.
