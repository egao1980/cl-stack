...
**Status:** **done** — [`egao1980/json-protocol`](https://github.com/egao1980/json-protocol) `0.2.0` + backends on GHCR. [`yaml-protocol`](https://github.com/egao1980/yaml-protocol) is a **separate repo** that **extends** JSON (`yaml-backend` ⊆ `json-backend`). Default YAML emit is **block**; `:style :json` is optional. JSON backends are optional for YAML. Cookbook [json.md](../cookbooks/json.md); `cl-stack-http` **0.1.8** uses jzon via protocol
