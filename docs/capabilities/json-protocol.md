...
**Status:** **done** — [`egao1980/json-protocol`](https://github.com/egao1980/json-protocol) `0.2.0` + backends on GHCR; `yaml-protocol` (same repo, **0.1.2**) **extends** JSON (`yaml-backend` ⊆ `json-backend`; JSON⊂YAML). Do not invert — `json-protocol` does not depend on YAML. Cookbook [json.md](../cookbooks/json.md); `cl-stack-http` **0.1.8** uses jzon via protocol
