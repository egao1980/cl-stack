...
| JSON / YAML / CSV / XML | Fragmented | P1 | JSON **shipped** — `json-protocol` **0.2.0** + jzon — [#91](https://github.com/egao1980/cl-stack/issues/91) · [json.md](cookbooks/json.md). YAML **shipped** — `yaml-protocol` (same Lisp mapping; serdes `:yaml`; pin **`latest`** until `:0.1.0` exists). CSV **shipped** — [`csv-protocol`](https://github.com/egao1980/csv-protocol) OCI **0.1.0** (serdes `:csv`/`:tsv`, dialects) · [csv.md](cookbooks/csv.md). XML **shipped** — `xml-protocol` **0.1.0** + `xml-backend-native` — [xml.md](cookbooks/xml.md) |
...
| Serdes | Fragmented | P2 | **shipped** — `serdes-protocol` **0.2.1** + JSONL/events; json / yaml / sexp / csv / xml / arrow / protobuf implement — [#132](https://github.com/egao1980/cl-stack/issues/132) · [serdes.md](capabilities/serdes.md) |
