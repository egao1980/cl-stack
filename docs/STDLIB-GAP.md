...
**Tags diagnose the CL ecosystem** (why a protocol or pin). They are not completeness. **Action** is the living scorecard — pins = [QUICKSTART](QUICKSTART.md) §2. Module-level backlog → [STDLIB-MODULES.md](STDLIB-MODULES.md). LLM leftovers → [AI-GAP.md](AI-GAP.md).

**Postponed:** multi-impl CI ([#42](https://github.com/egao1980/cl-stack/issues/42)).
...
| CLOS / MOP | Kernel-strong | P0 | GF protocols. `closer-mop` **1.0.0** pinned |
| Numbers / sequences | Kernel-strong | P2–P3 | Alexandria **1.0.1** pinned. Serapeum import staged (pin after OCI) |
...
| Sockets / DNS | De-facto converged | P1 | Consumed by `http-protocol`. `usocket` **0.8.9** pinned |
...
| Auth | Fragmented | P1 | **shipped** — `cl-stack-oauth2` **0.1.0** · `cl-stack-jwt` **0.3.3** (RS/ES/EdDSA; `expired-p` #6 closed) |
...
| Regex | De-facto converged | P2 | `cl-ppcre` **2.1.2** pinned |
...
| Protobuf / gRPC | Fragmented | P1 | **shipped** — `protobuf-protocol` **0.2.0** + cl-protobufs **0.2.0**; `grpc-protocol` **0.1.1** + http2 **0.3.1** (unary + interleaved bidi + gzip/deflate frames) / native — [protobuf.md](capabilities/protobuf.md) · [grpc.md](capabilities/grpc.md) |
| JSON / YAML / CSV / XML / MIME / CBOR / MessagePack / Avro / encodings | Fragmented | P1 | JSON **shipped** — `json-protocol` **0.2.0** + jzon — [#91](https://github.com/egao1980/cl-stack/issues/91) · [json.md](cookbooks/json.md). YAML **shipped** — `yaml-protocol` **0.2.0** (own repo; same Lisp mapping; serdes `:yaml`). CSV **shipped** — [`csv-protocol`](https://github.com/egao1980/csv-protocol) OCI **0.1.0** (serdes `:csv`/`:tsv`, dialects) · [csv.md](cookbooks/csv.md). XML **shipped** — `xml-protocol` **0.1.0** + `xml-backend-native` — [xml.md](cookbooks/xml.md). MIME **shipped** — `mime-protocol` **0.1.0** (serdes `:mime`/`:multipart`) · [mime-protocol.md](capabilities/mime-protocol.md). CBOR **shipped** — `cbor-protocol` **0.1.0** · [cbor-protocol.md](capabilities/cbor-protocol.md). MessagePack **shipped** — `messagepack-protocol` **0.1.0** · [messagepack-protocol.md](capabilities/messagepack-protocol.md). Avro **shipped** — `avro-protocol` **0.1.0** + `schema-protocol-avro` **0.1.1** · [avro-protocol.md](capabilities/avro-protocol.md). Encodings **shipped** — `encoding-protocol` **0.1.2** (RFC 4648 / QP / RLE; not HTTP CE) · [encoding-protocol.md](capabilities/encoding-protocol.md) |
| Schema | Fragmented | P2 | **shipped** — `schema-protocol` **0.2.0** (`emit-schema` / `parse-schema`) + `schema-protocol-json` **0.1.2** + `schema-protocol-xsd` **0.1.3** + `schema-protocol-arrow` **0.1.2** + `schema-protocol-avro` **0.1.1** — [schema.md](cookbooks/schema.md) |
| SQL | Fragmented | P2 | **shipped** — `sql-protocol` **0.1.0** · `sql-query{,-pg,-sqlite3}` **0.2.0** · `sql-orm` **0.1.0** · `sql-migrate` **0.1.0** (not Mito) — [#101](https://github.com/egao1980/cl-stack/issues/101) · [sql.md](cookbooks/sql.md) |
...
| Serdes | Fragmented | P2 | **shipped** — `serdes-protocol` **0.2.2** (media-type registry) + JSONL/events; json / yaml / sexp / csv / xml / arrow / protobuf / mime / cbor / messagepack / avro / encoding implement — [#132](https://github.com/egao1980/cl-stack/issues/132) · [serdes.md](capabilities/serdes.md) |
