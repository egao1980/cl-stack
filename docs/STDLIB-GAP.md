...
**Depth tranche ([#220](https://github.com/egao1980/cl-stack/issues/220) closed):** H2 `:want-stream` + trailers; request-body pipe + H2 DATA (`http-protocol` **0.3.7** / async **0.2.8**); gRPC http2 **interleaved** bidi + gzip/deflate frames (`grpc-backend-http2` **0.3.1**, live grpcio); crypto `sign`/`verify`; JWT RS/ES/EdDSA; H2 Clack backend **0.2.0** (`ENABLE_CONNECT_PROTOCOL`); WS H1 `accept` + **H2 Extended CONNECT server** (`ws-backend-websocket-driver` **0.4.0**). MIME / CBOR / MessagePack / Avro **shipped** as serdes implementors. Compressed gRPC frames **shipped**. Follow-on: HTTP/3. Do **not** start HTTP/3 or [AI-GAP](AI-GAP.md).
